// components/monitoring/LiveView.tsx
...
interface LiveViewProps {
  ...
  onPoseDetected?: (predictions: PosePrediction[]) => void
  modelBaseUrl?: string
...
export function LiveView({
  ...
  const emaAlpha = 1 // 0..1 (besar = responsif, kecil = halus)
  ...
  const loadModel = useCallback(async () => {
    ...
    try {
      ...
      // DEBUG: cek label kelas yang ke-load
      try {
        if (typeof (model as any).getClassLabels === 'function') {
          const labels = await (model as any).getClassLabels()
          console.log('TM model loaded from', modelURL, 'labels:', labels)
        } else {
          console.log('TM model loaded from', modelURL)
        }
      } catch (e) {
        console.warn('Could not read TM class labels:', e)
      }

      if (!procCanvasRef.current) procCanvasRef.current = document.createElement('canvas')
      setStatusText('Model ready')
      console.log('✅ TM Pose model ready:', modelURL)
    } catch (err) {
  ...
  const detectPose = useCallback(async () => {
    ...
    const minDelta = 1000 / Math.max(1, detectionFps)
    ...
    // ---- Sanity check: kalau keypoints hampir nggak ada → anggap Unknown ----
    let usedPreds: PosePrediction[] | null = null

    if (pose?.keypoints && pose.keypoints.length > 0) {
      const validPoints = pose.keypoints.filter((k: any) => (k.score ?? 0) > 0.3).length

      if (validPoints < 5) {
        // Tidak ada tubuh manusia yang jelas → Unknown full
        usedPreds = [
          { className: 'Unknown', probability: 1 },
          { className: 'Terlentang', probability: 0 },
          { className: 'Tengkurap', probability: 0 },
        ]
      }
    }

    if (!usedPreds && posenetOutput) {
      const rawPred: any[] = await model.predict(posenetOutput)

      // DEBUG: lihat raw output TM
      console.log('TM RAW preds:', rawPred)

      const mapped: PosePrediction[] = rawPred.map((p: any) => ({
        className: String(p.className ?? 'Unknown'),
        probability: Number(p.probability ?? 0),
      }))
      usedPreds = smoothPredictions(mapped)
    }

    if (usedPreds) {
      setPredictions(usedPreds)
      latestPredRef.current = usedPreds
    }

    // Gambar overlay (hanya kalau ada pose/keypoints yang layak)
          ...
          <div className="space-y-2">
            {predictions.slice(0, 3).map((p, i) => {
              ...
              return (
                ...
                >
                  <span className="font-medium text-gray-800 capitalize">{p.className}</span>
                  <div className="flex items-center gap-2">
