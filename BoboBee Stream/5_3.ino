...
void connectToWiFi(const char* ssid, const char* password) {
  ...
  // ====== ADD: Camera stream (tidak mengubah server kamu yang ada) ======
  if (CAM_initCamera()) {
    // Jalankan server kecil sendiri di :82 agar tak bentrok dengan server :80 milikmu
    if (CAM_startOwnServer(80)) {
      Serial.printf("[CAM] Stream URL: http://%s:82/stream\n",
                    WiFi.localIP().toString().c_str());
    } else {
      Serial.println("[CAM] start own server failed");
    }
  } else {
